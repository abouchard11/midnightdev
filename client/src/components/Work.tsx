  ...
  const projects = [
    {
      ...
      description: "Enterprise data visualization dashboard for financial institutions processing $5B+ annually.",
    ...
    },
    {
      ...
      description: "AI-driven lead generation system for a top-tier law firm, increasing qualified leads by 340%.",
    ...
    },
    {
      title: "NEXUS_HEALTH",
      category: "SaaS Platform",
      description: "HIPAA-compliant patient management system with integrated telemedicine capabilities.",
      ...
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <div key={index} className="group relative border border-white/10 bg-card hover:border-primary/50 transition-all duration-300">
              <div className="aspect-[4/3] w-full overflow-hidden border-b border-white/10 relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/60 backdrop-blur-sm">
                  <span className="font-mono text-primary text-lg tracking-widest border border-primary px-4 py-2">VIEW_CASE_STUDY</span>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <h3 className="text-xl font-bold mb-3 tracking-tight group-hover:text-primary transition-colors">{project.title}</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((t) => (
                    <Badge key={t} variant="secondary" className="rounded-none bg-white/5 text-white/70 hover:bg-white/10 font-normal text-xs">
                      {t}
                    </Badge>
                  ))}
                </div>
